...
def makemovie_mpl(
  ...
  Parameters:
    ...
    xmax      - the maximum lateral point in your data
                (typically (nx-1)*dx) [nx]
    ...
    ttlstring - title to be printed. Can be printed of the form
                ttlstring%(ottl + dttl*(framenumber))
  ...
  vmin = np.min(arr)
  vmax = np.max(arr)
  for ifr in tqdm(frames, ascii=True, desc="frames"):
    ...
    ax.imshow(
        ...
        extent=[
            kwargs.get("xmin", 0),
            kwargs.get("xmax", arr.shape[1]),
            kwargs.get("zmax", arr.shape[0]),
            kwargs.get("zmin", 0)
        ],
    ...
    if ('%' in kwargs.get('ttlstring', '')):
      ax.set_title(
          kwargs.get('ttlstring', '') %
          (kwargs.get('ottl', 0.0) + kwargs.get('dttl', 1.0) * k),
          fontsize=kwargs.get('labelsize', 18),
      )
...
def makemoviesbs_mpl(
  ...
  Parameters:
    arr1  - first input array (left panel) where the frames
            are on the fast axis
    arr2  - second input array (right panel) where the frames
            are on the fast axis
    odir  - the output directory where to save the frames
    ftype - the type (extension) of figure to save [png]
    qc    - look at each frame as it is saved [False]
  ...
  if arr1.shape[2] != arr2.shape[2]:
    raise ValueError("Both movies must have same number of frames")
  nfr = arr1.shape[2]
  k = 0
  frames = np.arange(0, nfr, skip)
  vmin1 = np.min(arr1)
  ...
  for ifr in tqdm(frames, ascii=True, desc="frames"):
    ...
    f, ax = plt.subplots(
        1,
        2,
        figsize=(kwargs.get("figsize1", 15), kwargs.get("figsize2", 8)),
        gridspec_kw={
            'width_ratios': [
                kwargs.get("wratio1", 1),
                kwargs.get("wratio2", 1)
            ]
        },
    )
    # First plot
    im1 = ax[0].imshow(
        arr1[:, :, ifr],
        cmap=kwargs.get("cmap1", "gray"),
        extent=[
            kwargs.get("xmin1", 0),
            kwargs.get("xmax1", arr1.shape[1]),
            kwargs.get("zmax1", arr1.shape[0]),
            kwargs.get("zmin1", 0)
        ],
        vmin=kwargs.get("vmin1", vmin1) * kwargs.get('pclip', 1.0),
        vmax=kwargs.get("vmax1", vmax1) * kwargs.get('pclip', 1.0),
        aspect=kwargs.get('aspect1', 1.0),
    )
    ax[0].set_xlabel(kwargs.get('xlabel1', ''),
                     fontsize=kwargs.get('labelsize', 18))
    ax[0].set_ylabel(kwargs.get('ylabel', ''),
                     fontsize=kwargs.get('labelsize', 18))
    ax[0].tick_params(labelsize=kwargs.get('ticksize', 18))
    if ('%' in kwargs.get('ttlstring1', '')):
      ax[0].set_title(
          kwargs.get('ttlstring1', '') %
          (kwargs.get('ottl1', 0.0) + kwargs.get('dttl1', 1.0) * k),
          fontsize=kwargs.get('labelsize', 18),
      )
    else:
      ax[0].set_title(kwargs.get('ttlstring1', ''),
                      fontsize=kwargs.get('labelsize', 18))
    # Second plot
    im2 = ax[1].imshow(
        arr2[:, :, ifr],
        cmap=kwargs.get("cmap2", "gray"),
        extent=[
            kwargs.get("xmin2", 0),
            kwargs.get("xmax2", arr2.shape[1]),
            kwargs.get("zmax2", arr2.shape[0]),
            kwargs.get("zmin2", 0)
        ],
        vmin=kwargs.get("vmin2", vmin2) * kwargs.get('pclip', 1.0),
        vmax=kwargs.get("vmax2", vmax2) * kwargs.get('pclip', 1.0),
        aspect=kwargs.get('aspect2', 1.0),
    )
    ax[1].set_xlabel(kwargs.get('xlabel2', ''),
                     fontsize=kwargs.get('labelsize', 18))
    ax[1].set_ylabel(kwargs.get('ylabel2', ''),
                     fontsize=kwargs.get('labelsize', 18))
    ax[1].tick_params(labelsize=kwargs.get('ticksize', 18))
    if ('%' in kwargs.get('ttlstring2', '')):
      ax[1].set_title(
          kwargs.get('ttlstring2', '') %
          (kwargs.get('ottl2', 0.0) + kwargs.get('dttl2', 1.0) * k),
          fontsize=kwargs.get('labelsize', 18),
      )
    else:
      ax[1].set_title(kwargs.get('ttlstring2', ''),
                      fontsize=kwargs.get('labelsize', 18))
    # Color bar
...
def makemovietb_mpl(
  ...
  Parameters:
    arr1  - first input array (Top panel) where the frames
            are on the fast axis
    arr2  - second input array (Bottom panel) where the frames
            are on the fast axis
    odir  - the output directory where to save the frames
    ftype - the type (extension) of figure to save [png]
    qc    - look at each frame as it is saved [False]
  ...
  for ifr in tqdm(frames, ascii=True, desc="frames"):
    ...
    im1 = ax[0].imshow(
        ...
        vmax=kwargs.get("vmax1", vmax1) * kwargs.get('pclip', 1.0),
        aspect=kwargs.get('aspect1', 1.0))
    ax[0].set_ylabel(kwargs.get('ylabel1', ''),
                     fontsize=kwargs.get('labelsize', 18))
    ax[0].set_xlabel(kwargs.get('xlabel1', ''),
                     fontsize=kwargs.get('labelsize', 18))
    ax[0].tick_params(labelsize=kwargs.get('ticksize', 18))
...
def viewimgframeskey(data, transp=True, fast=True, show=True, **kwargs):
  """
  Provides a frame by frame interactive viewing of a 3D
  numpy array via the arrow keys. Assumes the slow axis is the first axis.
  ...
  Parameters:
    ...
    cmap      - colormap [gray]
    vmin      - minimum value to display in the data
                [default is minimum amplitude of all data]
    vmax      - maximum value to display in the data
                [default is maximum amplitude of all data]
    ...
    interp    - interpolation type for better display of the data
                (sinc for seismic, bilinear of velocity) [none]
  ...
  def key_event(e):
    ...
    if ('%' in kwargs.get('ttlstring', ' ')):
      ax.set_title(
          kwargs.get('ttlstring', ' ') %
          (kwargs.get('ottl', 0.0) + kwargs.get('dttl', 1.0) * curr_pos),
          fontsize=kwargs.get('labelsize', 14))
    else:
      ax.set_title(kwargs.get('ttlstring', '%d' % curr_pos),
                   fontsize=kwargs.get('labelsize', 14))
  ...
  if (kwargs.get('scalebar', False)):
    ...
    cbar.draw_all()
  if ('%' in kwargs.get('ttlstring', ' ')):
    ax.set_title(kwargs.get('ttlstring', ' ') % (kwargs.get('ottl', 0.0)),
                 fontsize=kwargs.get('labelsize', 14))
  else:
    ax.set_title(kwargs.get('ttlstring', '%d' % curr_pos),
                 fontsize=kwargs.get('labelsize', 14))
...
def viewpltframeskey(data, ox=0.0, dx=1.0, transp=True, show=True, **kwargs):
  """
  Provides a frame by frame interactive viewing of a
  2D numpy array via the arrow keys. Assumes the slow axis is the first axis.
  ...
  if (kwargs.get('ymin', None) is None or kwargs.get('ymax', None) is None):
    ymin = np.min(data)
    ymax = np.max(data)
...
def viewcube3d(data,
  ...
  def key_event(e):
    ...
    if (ax[1, 0].get_xlim()[0] == os[0] and ax[1, 0].get_ylim()[1] == os[2] and
        ax[1, 1].get_xlim()[0] == os[1]):
      zoomed_out = True
    else:
      zoomed_out = False
    ...
    if e.key == "u" or e.key == "d":
      ...
      slc = bytes2float(data[curr_pos, i3, :, :]) if byte else data[curr_pos,
                                                                    i3, :, :]
      ax[0, 0].imshow(
          np.flip(slc, 0),
          interpolation=kwargs.get('interp', 'none'),
      ...
      slc = bytes2float(data[curr_pos, :, :,
                             i1]) if byte else data[curr_pos, :, :, i1]
      ax[1, 1].imshow(
          slc,
          interpolation=kwargs.get('interp', 'none'),
          aspect='auto',
  ...
  def onclick(e):
    ...
    if (tb.mode == ""):
      ...
      if (ax[1, 0].get_xlim()[0] == os[0] and
          ax[1, 0].get_ylim()[1] == os[2] and ax[1, 1].get_xlim()[0] == os[1]):
        zoomed_out = True
      else:
        zoomed_out = False
  ...
  def ondraw(e):
    ...
    if (plt.get_current_fig_manager().toolbar.mode == 'zoom rect' and
        updated is False):
  ...
  # yz plane
  slc = bytes2float(data[curr_pos, :, :, i1]) if byte else data[curr_pos, :, :,
                                                                i1]
  im = ax[1, 1].imshow(
      slc,
      interpolation=kwargs.get('interp', 'none'),
...
def resangframes(
  ...
  Parameters
    ...
    transp - flag indicating that the input image has shape
             [nro,na,nz,nx] [False]
  """
  if (transp):
  ...
  viewimgframeskey(
      ...
      xmax=kwargs.get('xmax', nx * dx),
      zmax=kwargs.get('zmax', nz * dz),
      show=show,
      xlabel='X (km)',
      ylabel='Z (km)',
  )
...
def viewresangptch(
  ...
  def key_event(e):
    ...
    iang = ang[curr_pos, :, :]
    iprb = prb[curr_pos]
    ax[0].set_title(r'$\rho$=%f prd=%g' % (oro + curr_pos * dro, iprb),
                    fontsize=kwargs.get('labelsize', 14))
    if (smb is not None):
      ax[1].set_title(r'smb=%g' % (smb[curr_pos]),
                      fontsize=kwargs.get('labelsize', 14))
  ...
  fig, ax = plt.subplots(
      1,
      3,
      figsize=(kwargs.get("wbox", 15), kwargs.get("hbox", 6)),
  )
  fig.canvas.mpl_connect('key_press_event', key_event)
  # Show the first frame
  istk = stk[0, :, :]
  ...
  l1 = ax[0].imshow(
      ...
      extent=[
          kwargs.get('xmin', 0.0),
          kwargs.get('xmax', img.shape[1]),
          kwargs.get('zmax', img.shape[0]),
          kwargs.get('zmin', 0.0)
      ],
  ...
  ax[0].tick_params(labelsize=kwargs.get('ticksize', 14))
  ax[0].set_title(r'$\rho=$%.4f prd=%g' % (oro, iprb),
                  fontsize=kwargs.get('labelsize', 14))
  l2 = ax[1].imshow(
      iang,
      cmap=kwargs.get('cmap', 'gray'),
  ...
  if (smb is not None):
    ax[1].set_title('smb=%g' % (smb[curr_pos]),
                    fontsize=kwargs.get('labelsize', 14))
  ax[2].plot(rhos, prb)
  if (smb is not None):
    ax[2].plot(rhos, smb)
...
def qc_f3data(
  ...
  def key_event(e):
    ...
    if e.key == "n":
      ...
      return
    #print("Beg={} End={} Cpos={} Nrec={}".format(beg,end,curr_pos,nrec[curr_pos]))
  ...
  scatr = ax[0].scatter(recx[0:nrec[0]],
                        recy[0:nrec[0]],
                        marker='v',
                        color='tab:green')
  ax[0].set_xlabel(kwargs.get('xlabel', 'X (km)'),
